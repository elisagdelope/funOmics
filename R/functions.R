  ...
  if (type == "pathifier") {
    ...
    for (i in seq_along(sets)) {
      ...
      if (type %in% c("mean", "median", "sd", "min", "max")) {
        ...
        stop(paste("Aggregation type", type, "is not supported. Please check the list of supported aggregation operators."))
      }
...
check_parameter <- function(param, name, expected_type = NULL) {
  if (is.null(param)) {
    stop(paste("The '", name, "' parameter must be specified and not NULL."))
  }
  if (!is.null(expected_type) && !inherits(param, expected_type)) {
    stop(paste("The '", name, "' parameter must be of type '", expected_type, "'."))
  }
}
...
aggby_pathifier <- function(X,gs) {
  pathifier_agg <- quantify_pathways_deregulation(data = as.matrix(X),
                                                  allgenes = rownames(X),
                                                  syms = gs, 
                                                  ...
                                                  attempts = 5,
                                                  min_exp = 0) # = remove effect if min_exp 

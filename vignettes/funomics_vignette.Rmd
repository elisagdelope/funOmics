...
print(paste("Pathway activity score matrix has dimensions:", nrow(pathway_activity_pca), ",", ncol(pathway_activity_pca)))
```
...
In this example, 18 of the gene sets in `kegg_sets` has size \< 12. You can check which pathways have been left out and how many genes they had by running the following command:

```{r}
length_sets <- sapply(kegg_sets, function(p) length(p))
...
As you can see, the resulting `gocc` object is a list of lists where each element represents a GO cellular compartment gene set.
